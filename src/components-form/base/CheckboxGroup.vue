      ...
      <div class="flex items-center gap-4">
        <div class="relative flex items-center">
          <input
            ...
            :value="item.value"
            :checked="dataValue?.includes(item.value)"
            @change="handleCheckboxChange(item.value, $event)"
            class="cb-input absolute opacity-0 w-0 h-0"
          />
          <div class="cb-box w-5 h-5 border-2 border-zinc-700 bg-zinc-800/50 rounded-md flex items-center justify-center transition-all duration-200">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="text-white transform scale-0 opacity-0 transition-all duration-200">
              <polyline points="20 6 9 17 4 12"></polyline>
            </svg>
          </div>
        </div>
        <div class="editor-item outline-none text-sm font-medium group-hover/item:text-zinc-100" :class="dataValue?.includes(item.value) ? 'text-zinc-300' : 'text-zinc-400'" :contenteditable="!isPreviewRender" @blur="changeValue($event, _index)">
          {{ item.label }}
        </div>
...
// 处理复选框变化
const checkedValues = ref<any[]>(Array.isArray(props.dataValue) ? props.dataValue : [])
...
const handleCheckboxChange = (value: any, event: Event) => {
  ...
  handleChange(checkedValues.value)
}
